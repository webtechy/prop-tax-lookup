...
def requires_notification(results):
    ...
    """
    # 1. Always alert if there is a known error or failure string
    for item in results:
        item_lower = item.lower()
        if "error" in item_lower or "could not find" in item_lower or "not found" in item_lower:
            return True

    # 2. Extract and check dollar amounts
    has_balance = False
    amounts_found = 0
    
    for item in results:
        # Finds patterns like $1,234.56 or $0.00
        matches = re.findall(r'\$([0-9,]+\.\d{2})', item)
        for match in matches:
            amounts_found += 1
            # Remove commas so we can convert it to a float safely
            numeric_val = float(match.replace(',', ''))
            if numeric_val > 0.0:
                has_balance = True
                
    # If we found amounts and at least one is > 0, we need to notify
    if has_balance:
        return True
        
    # If we found dollar amounts but none were > 0 (meaning all are $0.00), skip email
    if amounts_found > 0 and not has_balance:
        return False
        
    # Fallback: If we didn't find any recognizable dollar amounts, send an email to be safe
    return True
